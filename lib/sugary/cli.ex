defmodule Sugary.CLI do
  ...
  defp dispatch(["bench", "list" | rest]) do
    ...
    case Map.get(opts, "suite") do
      ...
      suite when suite in ["martian-offline", "cr-bench", "aacr-bench"] ->
  ...
  defp dispatch(["bench", "fetch", suite | rest])
  ...
  defp dispatch(["tool", "gauntlet" | rest]) do
    ...
    run_opts =
      [
        ...
        materialize: opts |> Map.get("materialize", "false") |> parse_bool()
      ]
  ...
  defp dispatch(["incumbent", "report" | rest]) do
    opts = parse_opts(rest)
    {:ok, Sugary.IncumbentLadder.report!(opts)}
  end

  defp dispatch(["pcrs", "claim", "refuter" | rest]) do
    opts = parse_opts(rest)
    {:ok, Sugary.ClaimRefuterGauntlet.run!(opts)}
  end

  defp dispatch(["pcrs", "staged", "replay" | rest]) do
    opts = parse_opts(rest)
    {:ok, Sugary.StagedPublisherReplay.run!(opts)}
  end

  defp dispatch(["pcrs", "transfer", "gate" | rest]) do
    opts = parse_opts(rest)
    {:ok, Sugary.TransferGate.run!(opts)}
  end

  defp dispatch(["pcrs", "portable", "transfer", "gate" | rest]) do
    opts = parse_opts(rest)
    {:ok, Sugary.PortableTransferGate.run!(opts)}
  end

  defp dispatch(["pcrs", "aacr", "v6", "gate" | rest]) do
    opts = parse_opts(rest)
    {:ok, Sugary.AACRV6Gate.run!(opts)}
  end

  defp dispatch(["orchestrator", "martian", "gate" | rest]) do
    opts = parse_opts(rest)
    {:ok, Sugary.MartianOrchestratorGate.run!(opts)}
  end

  defp dispatch(["h5i", "memory", "gauntlet" | rest]) do
    opts = parse_opts(rest)
    {:ok, Sugary.PersistentMemoryGauntlet.run!(opts)}
  end
  ...
  defp dispatch(["repo", "sparse-context" | rest]) do
    opts = parse_opts(rest)
    ...
    {:ok,
     Sugary.SparseRepoContext.run!(%{
       ...
       "id" => Map.get(opts, "id", "sparse-context-v0")
     })}
  end
  ...
  defp dispatch(_args) do
    {:error,
     "usage: sugary research init | sugary bench public list | sugary bench list [--suite <suite>] | sugary bench inspect --suite <suite> --limit <n> | sugary bench fetch <martian-offline|cr-bench|aacr-bench> --local-only | sugary bench run --suite <suite> --method <id> | sugary bench compare --run <dir> [--run <dir>] | sugary experiment run <manifest> [--replay-mode <mode>] | sugary experiment report <run-dir> | sugary campaign run <manifest> [--resume] [--dry-run] [--limit-experiments <n>] [--replay-mode <mode>] | sugary tool gauntlet --source-run <dir> --method <id> --baseline <id> [--tools a,b] [--materialize true|false] | sugary architecture gauntlet <manifest> [--replay-mode <mode>] | sugary scientific pilot --candidate <id|team.toml> --baseline <id|team.toml> [--experiment <manifest>] [--limit <n>] | sugary pcrs ensemble publisher [--limit <n>] | sugary incumbent report --source-run <dir> [--target-f1 0.70] | sugary pcrs claim refuter --source-run <dir> --materialization-run <dir> [--claim-limit <n>] | sugary pcrs staged replay --source-run <dir> --method <id> [--baseline <id>] [--suite martian-offline] [--limit <n>] [--offset <n>] | sugary pcrs transfer gate [--suite aacr-bench] [--limit <n>] | sugary pcrs portable transfer gate [--suites martian-offline,aacr-bench] [--limit <n>] | sugary pcrs aacr v6 gate [--limit <n>] [--replay-mode cache-first] | sugary orchestrator martian gate [--limit <n>] [--replay-mode cache-first] [--agent-bus auto|local-jsonl|h5i] | sugary h5i memory gauntlet [--train-limit <n>] [--eval-limit <n>] [--h5i true|false] | sugary repo materialize --suite <suite> [--mode plan|metadata|fetch] [--limit <n>] | sugary repo sparse-context --suite <suite> [--limit <n>] | sugary martian parity export --source-run <dir> --method <id> [--policy team-ev-max-2] | sugary martian no-key report [--sugary-tool <tool>] | sugary team search --pack <pack> --suite <suite> --max-team-size <n> | sugary reviewers check --pack <pack> | sugary promotion lock --candidate <path> --suite <suite> --dev-run <run-dir> --out <path> [--baseline-method <id>] [--baseline-team <path>] | sugary promotion run <lock> --split holdout"}
  end
  ...
  defp parse_bool(value) when value in ["false", "FALSE", "0", "no", "NO"], do: false
